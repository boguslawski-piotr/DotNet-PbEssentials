...
namespace pbXNet.Database
{
	...
	s = _sql
		.Delete().From("Things").Where().E(_sql.New().Expr()["StorageId"].Eq.P(1));
		...
		.Create().Table("Things2")
			...
			["Data"].NText().Null()
				...
				.CConstraint("DataNotNull")
					.Check(_sql.New().Expr()["Data"].Is.NotNull().And()["Data"].Neq.E("func('a')"))
			["ModifiedOn"].T("bigint").NotNull()
				.Check(_sql.New().Expr()["ModifiedOn"].Neq.E("0"))
			.Constraint("PK_Things2")
				.PrimaryKey()["StorageId"]["Id"]
			.Constraint()
				.Unique()["Id"]
			.Constraint()
				.Check(_sql.New().Expr()["Id"].Neq.E("0"));
	...
	*/
	...
	public class SqlBuilder
	{
		...
		public virtual string AutoGeneratedParameterPrefix => "__";
		...
		public virtual SqlBuilder Expr() => Start(null).WhereModeOn();
		public virtual SqlBuilder Expr(string expr) => Start(null).WhereModeOn().E(expr);

		public virtual SqlBuilder Create() => Start("CREATE");
		public virtual SqlBuilder Drop() => Start("DROP").DropModeOn();
		public virtual SqlBuilder Update(string what) => Start("UPDATE").Add(what, false, true).Add("SET", false, true).UpdateModeOn();
		public virtual SqlBuilder InsertInto(string where) => Start("INSERT INTO").Add(where, false, true).OpenBracket().InsertModeOn();
		...
		public virtual SqlBuilder Table(string name) => Add("TABLE").AddIfTrue(_dropMode, New().IfExists()).Add(name, true, true).OpenBracketIfTrue(!_dropMode).ValuesModeOn();
		...
		public virtual SqlBuilder On(string tableName) => (_dropMode && !DropIndexStmtNeedsTableName ? this : Add("ON", true, true).Add(tableName, true, true).OpenBracketIfTrue(!_dropMode).ValuesModeOn());
		...
		public virtual SqlBuilder Values() => DelLastComma().CloseBracket().Add("VALUES", true, true).OpenBracket().ValuesModeOn();

		public virtual SqlBuilder From(string src) => DelLastComma().Add("FROM", true, true).Add(src, false, true);

		public virtual SqlBuilder Where() => DelLastComma().WhereModeOn().Add("WHERE", true, true);
		public virtual SqlBuilder OrderBy() => WhereModeOff().Add("ORDER BY", true, true).OrderByModeOn();
		public virtual SqlBuilder GroupBy() => WhereModeOff().Add("GROUP BY", true, true).GroupByModeOn();

		public virtual SqlBuilder Text(string freeText) => Add(freeText, true, true);

		public SqlBuilder P(int num) => P(AutoGeneratedParameterPrefix + num.ToString());
		...
		public virtual SqlBuilder Count(string expr = null) => Add("COUNT", true, false).Ob().Add(expr ?? "*").Cb().Add(" ");
		public virtual SqlBuilder Sum(string expr) => Add("SUM", true, false).Ob().Add(expr).Cb().Add(" ");
		public virtual SqlBuilder Min(string expr) => Add("MIN", true, false).Ob().Add(expr).Cb().Add(" ");
		public virtual SqlBuilder Max(string expr) => Add("MAX", true, false).Ob().Add(expr).Cb().Add(" ");
		public virtual SqlBuilder Avg(string expr) => Add("AVG", true, false).Ob().Add(expr).Cb().Add(" ");
		...
		{
			_sql = new StringBuilder(src._sql.ToString(), 256);
		...
		public SqlBuilder Start(string arg)
		{
			_sql.Clear();
			_currentColumn = 0;
			_dropMode = false;
			...
			return arg != null ? Add(arg, false, true) : this;
		}
		...
		public bool ReplaceSelectColumnsWith(string replacement)
		{
			SqlBuilder tempSqlBuilder = New();

			string select = tempSqlBuilder.Select().Build().Trim();

			string sql = Build();

			// TODO: handle: SELECT DISTINCT and SELECT ALL

			int sidx = sql.IndexOf(select, StringComparison.OrdinalIgnoreCase);
			if (sidx >= 0)
			{
				string from = tempSqlBuilder.Start(null).From(null).Build().Trim();

				int fidx = sql.IndexOf(from, StringComparison.OrdinalIgnoreCase);
				fidx = fidx < 0 ? sql.Length - 1 : fidx;

				sql = sql
					.Remove(sidx + select.Length, fidx - (sidx + select.Length))
					.Insert(sidx + select.Length, " " + replacement + " ");
		...
		protected SqlBuilder DropModeOff() => OnOff(ref _dropMode, false);
		protected SqlBuilder DropModeOn() => OnOff(ref _dropMode, true);
		...
		protected SqlBuilder UpdateModeOff() => OnOff(ref _updateMode, false);
		protected SqlBuilder UpdateModeOn()
		{
			_updateMode = true;
			_currentColumn = 0;
			return this;
		...
		protected SqlBuilder InsertModeOff() => OnOff(ref _insertMode, false);
		protected SqlBuilder InsertModeOn() => OnOff(ref _insertMode, true);

		protected SqlBuilder ValuesModeOff() => OnOff(ref _valuesMode, false);
		protected SqlBuilder ValuesModeOn() => OnOff(ref _valuesMode, true);

		protected SqlBuilder WhereModeOff() => OnOff(ref _whereMode, false);
		protected SqlBuilder WhereModeOn()
		{
			_whereMode = true;
			_updateMode = false;
			return this;
		...
		protected SqlBuilder OrderByModeOn() => OnOff(ref _orderbyMode, true);
		protected SqlBuilder GroupByModeOn() => OnOff(ref _groupbyMode, true);
